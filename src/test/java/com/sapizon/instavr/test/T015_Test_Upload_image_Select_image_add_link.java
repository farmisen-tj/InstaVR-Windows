...
public class T015_Test_Upload_image_Select_image_add_link extends TestBase {
	...
	public void AddLink(String Email, String password, String runmode) throws Exception {
		...
		if (driver.findElement(By.xpath(Configration.Assert)) != null) {
			...
			captureScreen("");
			test.log(LogStatus.INFO, "Verify the Link added sucessfully" + test.addScreenCapture(newFileName));
		} else {
			captureScreen("");
			test.log(LogStatus.INFO, "Unable To Locate Link button" + test.addScreenCapture(newFileName));
		}
